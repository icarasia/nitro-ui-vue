  ...
  >
      ...
      <tr>
        ...
        >
          ...
          <i
            ...
            :class="{
              [iconClass]: iconClass,
              [header.sort || 'asc']: true
            }"
            >{{ header.sort === "asc" ? "DESC" : "ASC" }}</i
          >
        </th>
      </tr>
    </thead>
    <tbody>
      <template v-if="!$slots.body">
        ...
        >
          <td
            v-for="(headerItem, key1) in headers"
            :key="`cell-${key1}`"
            :class="{
              [`text-${headerItem.align}`]: headerItem.align
            }"
          >
...
export default {
  ...
  methods: {
    sortBy(index, params) {
      params.sort =
        params.sort && params.sort.toLowerCase() === "asc" ? "desc" : "asc";
