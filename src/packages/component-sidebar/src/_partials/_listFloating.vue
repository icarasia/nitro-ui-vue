<template>
    <div class="c-sidebar__floating-menu  u-fixed  u-zindex-1 floating-sidebar--menu-hide"
         :id="`submenu-${index}`"
         :style="{
            top  : `${top}px`,
            left : `${left}px`,
         ...
         @mouseover="$emit('update:hovered', index);show = true"
         @mouseleave="$emit('update:hovered', null);show = false"
    >
        <a-link :to="(!menuItem.subItems || !menuItem.subItems.length) ? menuItem.to : null"
                class="no--underline">
            <span class="c-sidebar__label  u-color-white  u-padding-ends-sm  u-padding-sides-lg  u-hidden">
                <span class="u-margin-left-sm">{{ menuItem.text }}</span>
            </span>
        </a-link>
        <ul class="o-list  u-padding-top-sm  u-padding-bottom-xxs" v-if="menuItem.subItems && menuItem.subItems.length">
